...
| L2 | 長話口閉じの表示 clock 最小修正 | `pass` | 2026-09-11（Pass-with-note。seam A 消滅。負の audio_ms で前ターン FG 連続） |
...
- P1 Keep: `--prompt_dir` 2系統（`prompts_en`＝20あり30空 / `prompts_en_battle`＝20空・30=trash_talk）。interrupt/leadership は今の prompt_dir 横。JP fallback。口 barge-in＝既存 talkover `clear_queue`（mute は切らない）。二重 InputStream は監視。`main` FF 済（`f117d51` / `phase-p1-pass`）
- P1b Keep: battle `00` 許可 emo は `1_1` / `1_2` / `2_1` / `2_2` / `3_1` / `9_1` のみ。`2_0` surprise 禁止。YAML は今フェーズ未変更。`main` FF 済（`34458fb` / `phase-p1b-pass`）
...
- L1 指紋 A（seam の VC `audio_ms=0`）は **L2 で閉じた**。N↑・ジッタ延長・虐殺短文化・バースト専用は出さない
- 141259 のバースト HOL／中盤 REB（L2 対象外のまま）
- L2 観察の 1 回目 BG/FG 顔上下ずれ（既知。B 再開しない）
...
**Pass 基準:**
- [x] 分割が 10＝Role&Persona＋Tone&Style、30＝Principles 1–9＋Few-Shot＋until interrupted
- [x] emo 許可がロックどおり。`2_0` surprise なし。参考の全10系統を `00` に貼っていない
- [x] Keep 非破壊（P1 2系統 / V1 Aoede / 方式2 / 図A / N=2 / jitter 300/240 / silence 350 / B7 / E1b）
- [x] Zoom 主観は親（device 再クエリ後成立。番号ずれの初回は Fail にしない）

**子報告要約（2026-09-10）:**
- 変更は battle `00` / `10` / `30` のみ。`20` 空。SHA256 一致。few-shot 7例。until interrupted 残置
- 主観 134634 / 134744 / 134904 は mic=1=WebCamera・audio=15=CABLE In 16ch で `has_spoken=False`（番号ずれ。device 再クエリ Keep）
- 再クエリ後 141259（mic=8 USB / audio=23 ヘッドホン）4t 成立。Taxi Driver `[emo:2_1]`。本文一致
- 観察（P1b 外）: 初手 Live first PCM 遅延（送信後約22s）／ turn2 口閉じ 5–8s（`audio_ms=0` で前ターン閉じ口 FG）

**親判定（2026-09-10）:**
- **Pass-with-note。** 分割と emo ロックは成立。番号ずれは Keep（永久決め打ち禁止）の再確認であり Fail にしない
- 出さない: バースト専用処理、N↑、ジッタ延長、fast/slow、先行 enqueue、YAML 修正、B 再開
- 再発時の疑い順のみ記録: mouth 前線 → 到着順 enqueue 頭止め → 否定後に M0 N
- Keep All（commit / tag `phase-p1b-pass`）は **親が実施**。子はしない。`in/*.txt` は入れない。`main` マージは別依頼
- 次本線=なし（指名待ち）→ **L1**（2026-09-10 着手。計測のみ）

---

## Phase L1: 長話口閉じ計測（実装なし）

**目的:** 自己対戦の長話で「数秒〜5s 口閉じ、同ターン復旧、再発」をログ指紋に落とす。A/B と疑い順まで。実装しない。

**作業ブランチ:** 新ブランチ不要。コード変更禁止。計測子のみ。

**確定事実（覆すな）:**
- P1b Pass-with-note。虐殺（一方通行）は今は直さない。割り込み・主導権はこの子に混ぜない
- 141259 は初手バースト＋ clock/FG。自己対戦の長話は別指紋（同じターン内で復旧→再発）
- 図A: Live のドカン到着は想定内。バースト専用処理・N↑・ジッタ延長は第一手段にしない
- 疑い順: (1) mouth 前線 `wait_mouth` / coverage (2) 到着順 enqueue 頭止め `order_wait` / HOL (3) 上を否定してから M0 N（万能ではない）

**対象ログ（消すな・外出しから戻すな）:**
- 長話口閉じ: `logs/sess_en_live1_subj_20260910_162939.log`
- 長話口閉じ: `logs/sess_en_live1_subj_20260910_163855.log`
- 初手バースト対照: `logs/sess_en_live1_subj_20260910_141259.log`
- 短文・互いに被せ・口閉じなし対照: `logs/sess_en_live1_subj_20260910_143450.log`

**スコープ（子）:**
1. 主観をログ指紋へ
2. (A) 口閉じ＋音声継続（PLAYING なのに virtualcam `shown_fg` / `audio_ms` 停滞）か (B) 音声も途切れ（REBUFFERING / UNDERRUN）か。混在なら時系列
3. 141259 と同型か、長話側だけか
4. 疑い順 (1)→(2)→(3)。閉じられなければ「未閉じ」。推測実装を提案するな

**スコープ外:** session_loop / M0 / player / prompts / YAML / interrupt・leadership、N↑、ジッタ、バースト専用、先行 enqueue、frame drop、口形捨て、虐殺を短くする prompt、`--turns` 尺制御、IDLE 廃止、PROGRESS、commit / tag

**Pass 基準:**
- [x] 指紋が A/B（または混在）で閉じている
- [x] 疑い順の結論がログ根拠つき（未閉じ可）
- [x] 実装提案があっても親承認なしに書いてよいのは「次に測る1項目」まで。パッチ案・N↑案は出さない

**子報告要約（2026-09-10）:**
- 長話は **A**。冒頭だけ短い B。162939 は fo=567/744/951 で +3.0/+2.5/+2.0s 後復旧。163855 は fo=624 +7.9s 復旧、fo=861 は復旧なし（ターン全体 A）
- 再発は各長ターン開始 seam。同一 fo 中盤の二度 stall は VC 上なし。143450 の seam am=0 は +0.2s
- 141259 と A の clock/FG は共有。バースト HOL（ow=20820）と中盤 REB は 141259 だけ
- (1) wait_mouth 否定（A 中 max_wm=0）(2) order_wait/HOL 否定（長話 ow max=28）(3) M0 N 否定（pending 秒単位でも REB なし＝供給負けなら B）
- 次に測る1項目（未着手）: 各 A 区間の `enqueue_timeline_end_ms` vs VC `audio_ms`

**親判定（2026-09-10）:**
- **Pass-with-note。** 定番3否定 ≠ 原因不明。残りは VC 表示 clock（seam で `audio_ms=0` が追いつかない）
...
**Pass 基準:**
- [x] 長話相当で seam の数秒 A（口閉じ＋音声継続）が Before 比で消えるか、残るなら計測で閉じる
- [x] 短文 talkover（143450）を壊さない
- [x] Keep 非破壊（図A／方式2／N=2／jitter 300/240／E1b override／P1b）
- [x] 親向けサマリーのみ

**子報告要約（2026-09-11）:**
- 原因: `_resolve_ssot_target` が `max(0, played−base)`。leftover pending のあいだ PLAYING でも `audio_ms=0` / `shown_fg=frame_offset`
- 修正: `audio_ms = player_local_ms − base_ms`（0 クランプなし）。負は前ターン FG 連続。runtime は `run_virtualcam_persistent.py` のみ
- After 長話 `221154`: 指紋 A=0。T2 leftover 4.3s は `am=-4226` / shown=221（fo ではない）。T3 同様
- 対話 `220653`: 口閉じフリーズなし。A は T3 onset の B6 1 tick のみ
- 143450 再計算: 200ms pending は `am=-44` / 1 コマ。B3hf / B3hf2 / phase7 selfcheck PASS

**親判定（2026-09-11）:**
- **Pass-with-note。** seam の数秒 A は Before 比で消滅。主観 220653 / 221154 一致
- Fail にしない: 1 回目 BG/FG 顔上下ずれ（既知）。対話末尾の短い REB/UNDERRUN（141259 HOL ではない）
- 出さない: N↑、ジッタ延長、バースト専用、audio_ms 全体オフセット、catch-up 先出し、B 再開、次本線
- Keep All（commit / tag `phase-l2-pass`）は **親が実施**。`in/*.txt`・logs は入れない。`main` FF 済（2026-09-11。`e37893a`→`523f7f6`。D1 は入れていない）
- 次本線=**Q1**（2026-09-12 着手。起動時デバイス名前再クエリ）

---

## Phase Q1: 起動時デバイス名前再クエリ

**目的:** 第三者が MME 番号を覚えず、経路（手元 USB / Zoom）を選んで起動できる。番号は起動のたびに名前解決し、**メモリにだけ持つ**。ファイルに焼くな。

**作業ブランチ:** `feature/device-name-query`（from `main` `f8b76a1`）。D1（`feature/ops-third-party`）は載せるな。

**確定事実（覆すな）:**
- 番号永久固定禁止。Windows 更新で MME はずれる。第三者に番号手書きをさせない
- 経路は **名前セット** だけ持つ。採用は起動のたびに `query_devices()`（**MME**）。前回番号を使わない
- Zoom 出＝名前に **「CABLE Input」**（「CABLE In 16ch」ではない）。Zoom 入＝名前に **「Voicemeeter Out B1」**（「CABLE Output」ではない）
- 手元 USB 出／入＝名前に **「USB PnP」** の speaker / mic（「WebCamera」ではない）
- 0件または2件以上、Banana 未起動で B1 無し → **起動しない**。推測しない。一覧を出して止める
- 画面に今回の解決番号を **読み取り専用** 表示してよい。編集させない
- CLI `--audio_device` / `--mic_input_device` は **開発者上書きのみ**。第三者経路はプロファイルだけ
- 管理画面に Live を埋め込まない。バトル開始ボタン・連続バトル・壁時計・割り込みゲートは出さない
- `docs/ops_zoom_third_party.md` は再発行しない。D1 を main に載せない

**スコープ（子）:**
1. プロファイル→名前セット→MME 1件一致→そのプロセスの device index
2. 失敗時は起動拒否＋理由（一覧付き）。推測マッチ禁止
3. 解決結果をログと管理画面の読み取り専用に出す
4. 呼び出しは session_loop 起動時と admin。図A・player・M0・prompt は触るな

**スコープ外（Q1 禁止）:**
- 番号の SSOT ファイル（`in/*.txt` / `configs` に index を書くな）
- CABLE を Hardware In、session_loop ミックス、Streamlit `asyncio.run` で Live、二重 Live
- N↑、ジッタ延長、session_loop sleep、talkover 変更、口 barge-in、B/O 再開
- ops_zoom 再発行、D1 merge、Q1 以外の残り開発
- `docs/PROGRESS.md` 編集。commit / tag / Keep All（親がやる）

**Pass 基準:**
- [x] Zoom / 手元 USB で名前解決が1件になり、起動前 bind にその index が入る
- [x] Ambiguous / 欠落で起動しない（一覧＋理由。失敗はフィクスチャ可）
- [x] 番号を `in/*.txt` や `configs` に書いていない
- [x] 図A / 方式2 / L2 / P1b / E1b 非破壊
- [x] 親向けサマリーのみ

**子報告要約（2026-09-12）:**
- ヘルパ `audio_device_name_query.py`。session_loop は `--audio_route_profile`＋`asyncio.run(_run)` 前 bind。admin は経路ラジオ＋`st.code` 読み取り専用
- この機: Zoom 出 `CABLE Input` **23** / 入 `Voicemeeter Out B1` **10**。手元 USB 出 **17** / 入 **1**
- 失敗 rc=2: 0件 / 曖昧 / B1 無し。推測なし。番号ファイルなし
- 証拠範囲: Live `_run` 未起動。失敗はフィクスチャ。admin ラジオは session_loop に渡さない。プロファイル未指定は旧 default `--audio_device 15`

**親判定（2026-09-12）:**
- **Pass-with-note。** 名前1件→起動前 bind、拒否＋一覧、非永続は成立。Live 主観は Q1 Gate にしない
...
- 次本線=**G1**（2026-09-12 着手。管理画面「今すぐ割り込め」PLAYING ゲート）

---

## Phase G1: 管理画面「今すぐ割り込め」PLAYING ゲート

**目的:** 第三者の誤押し防止。猫が鳴っている最中に定型 interrupt を投げない。「今すぐ割り込め」が PLAYING 中なら no-op。

**作業ブランチ:** `feature/admin-interrupt-gate`（from `main` `03915ea`＝`99b4011`＋Q1 FF記録）。D1（`feature/ops-third-party`）は載せるな。

**確定事実（覆すな）:**
- 対象ボタンは **「今すぐ割り込め」だけ**。主導権奪取 / Zoom主導権 / 主導権解除 OPEN は触るな
- 口 barge-in＝既存 talkover。mute しない。**G1 で触るな・ゲートするな**
- PLAYING **確定**のときだけ破棄。`in/battle_interrupt_live.txt` に書くな。UI に破棄と出す
- IDLE / BUFFERING / REBUFFERING / UNKNOWN / 状態ファイル無し・読めない → **破棄するな**。今どおり `write_interrupt`
- 状態は既存 player の `playback_state`（猫の再生 state）。壁時計・session_loop sleep・第二時計を作るな
- 管理画面に Live を埋め込むな。連続バトル・壁時計尺・定型追記・自己対戦改善は出さない
- Q1 Keep: `--audio_route_profile`。番号をファイルに焼くな。D1 を main に載せない。ops_zoom 再発行しない

**スコープ（子）:**
1. `admin_control_panel.py` の「今すぐ割り込め」だけ。必要なら状態を読む最小ヘルパ
2. 既存 `playback_state` を読んで PLAYING なら `write_interrupt` しない
3. 破棄ログと画面表示。ファイル未更新

**スコープ外（G1 禁止）:**
- talkover / session_loop の interrupt 消費 / 主導権 / Zoom主導権
- 図A / player / M0 / prompt / YAML / Q1 改変
- ②壁時計／③連続／④定型追記／⑤⑥
- Streamlit `asyncio.run` で Live、session_loop sleep、N↑、ジッタ延長、text trigger 復活、虐殺短文化
- `docs/PROGRESS.md` 編集。commit / tag / Keep All（親がやる）

**Pass 基準:**
- [x] PLAYING 中はファイル未更新＋破棄表示
- [x] 非 PLAYING（IDLE / BUFFERING / REBUFFERING / UNKNOWN / 無し・読めない）は今どおり投入
- [x] talkover / 主導権 / 図A / Q1 / L2 / P1b / E1b 非破壊
- [x] 親向けサマリーのみ。Live 主観は Gate にしない

**子報告要約（2026-09-12）:**
- admin「今すぐ割り込め」のみ `write_interrupt_unless_playing`。`write_interrupt` 本体未改変
- PLAYING: ファイル `SENTINEL_UNCHANGED`／未作成。ログ `[admin][interrupt_discard]`。UI 破棄表示
- 非 PLAYING（IDLE / BUFFERING / REBUFFERING / UNKNOWN / 無し / 読めない / 空）: `write_interrupt`（text / battle / 60s 据え置き）
- 主導権／Zoom主導権は従来関数。PLAYING でも leadership は interrupt を書く。talkover / session_loop / player 非触
- Q1 selfcheck PASS。L2 / P1b / E1b 非触
...
| 2026-09-11 | Phase L2 Pass-with-note。0 クランプ撤去で seam A 消滅。221154 / 220653 主観一致。Keep All は親。次本線なし |
