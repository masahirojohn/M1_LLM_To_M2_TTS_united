...
from aituber_llm_tts.prompt_builder import (
    build_input_summary_dict,
    build_priority_inputs_dict,
)
from aituber_llm_tts.m1_output_parser import parse_m1_output_text
...
def main() -> None:
    ...
    ap.add_argument(
        "--raw_output_json",
        default="examples/m1_raw_output_ok.json",
    )
    ap.add_argument(
        "--max_comments",
        type=int,
    ...
    input_summary = build_input_summary_dict(state=state, source=args.source)
    priority_inputs = build_priority_inputs_dict(state=state)

    raw_text = Path(args.raw_output_json).read_text(encoding="utf-8")
